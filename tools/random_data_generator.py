...
class DataGenerator:
    ...
    def generate_data_for_column(self, col: Column) -> List:
        return [get_random_choice_for_type(col.d_type) for row in range(self.table.rows)]

    def generate_data_for_table(self) -> List[List]:
        return [self.generate_data_for_column(column) for column in self.table.column_info]
